...
class MusicCollectionApp {
    ...
    showAutocomplete(containerId, items, field) {
        ...
        items.forEach(item => {
            if (item && item[field] && item[field] !== 'undefined') {
                ...
                // For albums, show title and year if available
                if (field === 'album_name' && item.year) {
                    div.textContent = `${item[field]} (${item.year})`;
                } else {
    ...
    showAutocompleteLoading(containerId) {
        ...
        if (list) {
            ...
            console.error('Could not find autocomplete list for:', containerId);
        }
    }
    ...
    hideAutocomplete(containerId) {
        ...
        if (list) {
            ...
            console.log('No list found to hide for:', containerId);
        }
    }
    ...
    renderAlbums(albums) {
        ...
        tbody.innerHTML = albums.map(album => `
            <tr data-id="${album.id}">
                <td class="cover-cell">
                    ${album.cover_url && album.cover_url.trim() !== '' ? 
        ...
        try {
            ...
            if (data.success) {
                ...
                if (data.auth_required) {
                    ...
                    this.showModalMessage(data.message, 'error');
                }
            }
        } catch (error) {
            console.error('Error saving album:', error);
            this.showModalMessage('Error saving album', 'error');
        }
    }
    ...
    loadOptimizedImage(img, src, fallbackSrc = null) {
        ...
        if (webpSupported && src.includes('discogs.com') && !src.includes('fm=webp')) {
            // Add WebP format to Discogs URLs
