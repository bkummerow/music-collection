...
updateStats(stats){document.getElementById('modalTotalAlbums').textContent=stats.total_albums||0;document.getElementById('modalOwnedAlbums').textContent=stats.owned_count||0;document.getElementById('modalWantedAlbums').textContent=stats.wanted_count||0;document.getElementById('modalUniqueArtists').textContent=stats.unique_artists||0;const styleStatsList=document.getElementById('styleStatsList');if(styleStatsList&&stats.style_counts){const styleEntries=Object.entries(stats.style_counts);if(styleEntries.length>0){const topStyles=styleEntries.slice(0,10);styleStatsList.innerHTML=topStyles.map(([style,count])=>`
              ...
              `).join('')}else{styleStatsList.innerHTML='<p class="no-styles">No style information available</p>'}}}
