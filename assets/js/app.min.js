...
const modal=document.getElementById('viewRecordModal');const recordData=document.getElementById('recordData');try{const response=await this.fetchWithCache(`api/music_api.php?action=album&id=${this.editingAlbum.id}`);const data=await response.json();if(data.success&&data.data){const formattedData=JSON.stringify(data.data,null,2);recordData.textContent=formattedData}else{recordData.textContent='Error loading album data: '+(data.message||'Unknown error')}}catch(error){recordData.textContent='Error loading album data: '+error.message}
...
recordData.contentEditable=!1;editBtn.style.display='block';saveBtn.style.display='none';cancelBtn.style.display='none';editWarning.style.display='none';editError.style.display='none';this.originalRecordData=null}
async saveRecordChanges(){const recordData=document.getElementById('recordData');const editBtn=document.getElementById('editRecordBtn');const saveBtn=document.getElementById('saveRecordBtn');const cancelBtn=document.getElementById('cancelEditBtn');try{const editedData=JSON.parse(recordData.textContent);if(!editedData.artist_name||!editedData.album_name){throw new Error('Artist name and album name are required')}
