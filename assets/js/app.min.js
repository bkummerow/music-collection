...
tbody.innerHTML=albums.map(album=>`
      ...
      `).join('');this.initLazyLoading();this.fetchMasterYears(albums)}
async fetchMasterYears(albums){const albumsWithDiscogsId=albums.filter(album=>album.discogs_release_id);if(albumsWithDiscogsId.length===0){return}
try{const albumIds=albumsWithDiscogsId.map(album=>album.id).join(',');const response=await this.fetchWithCache(`api/music_api.php?action=master_years&album_ids=${albumIds}`);const data=await response.json();if(data.success&&data.data){Object.keys(data.data).forEach(albumId=>{const masterYear=data.data[albumId];const yearBadges=document.querySelectorAll(`tr[data-id="${albumId}"] .year-badge`);yearBadges.forEach(badge=>{if(badge.textContent!==masterYear.toString()){badge.textContent=masterYear}});const coverImage=document.querySelector(`tr[data-id="${albumId}"] .album-cover`);const albumLink=document.querySelector(`tr[data-id="${albumId}"] .album-link`);if(coverImage){coverImage.dataset.year=masterYear}
if(albumLink){albumLink.dataset.year=masterYear}})}}catch(error){console.log('Error fetching master years:',error)}}
