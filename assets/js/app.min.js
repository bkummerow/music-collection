class MusicCollectionApp{constructor(){this.currentFilter='all';this.currentSearch='';this.currentStyleFilter='';this.editingAlbum=null;this.autocompleteTimeout=null;this.artistAutocompleteTimeout=null;this.albumAutocompleteTimeout=null;this.isAuthenticated=!1;this.init()}
...
this.updateAuthUI()}
updateAuthUI(){const addBtn=document.getElementById('addAlbumBtn');const loginBtn=document.getElementById('loginBtn');const logoutBtn=document.getElementById('logoutBtn');if(addBtn){if(this.isAuthenticated){addBtn.textContent='+ Add Album';addBtn.style.opacity='1';addBtn.style.cursor='pointer'}else{addBtn.textContent='+ Add Album (Login Required)';addBtn.style.opacity='1';addBtn.style.cursor='pointer'}}
...
setFilter(filter){this.currentFilter=filter;this.currentStyleFilter='';document.querySelectorAll('.filter-btn').forEach(btn=>{btn.classList.remove('active')});document.querySelector(`[data-filter="${filter}"]`).classList.add('active');this.loadAlbums()}
filterByStyle(style){this.hideStatsModal();this.currentStyleFilter=style;const searchInput=document.getElementById('searchInput');const clearSearchBtn=document.getElementById('clearSearch');if(searchInput){searchInput.value=`Style: ${style}`;searchInput.disabled=!0;clearSearchBtn.classList.add('visible')}
this.showMessage(`Filtering by style: ${style}`,'info');this.loadAlbums()}
clearStyleFilter(){this.currentStyleFilter='';const searchInput=document.getElementById('searchInput');if(searchInput){searchInput.value='';searchInput.disabled=!1}
this.loadAlbums()}
...
updateStats(stats){document.getElementById('modalTotalAlbums').textContent=stats.total_albums||0;document.getElementById('modalOwnedAlbums').textContent=stats.owned_count||0;document.getElementById('modalWantedAlbums').textContent=stats.wanted_count||0;document.getElementById('modalUniqueArtists').textContent=stats.unique_artists||0;const styleStatsList=document.getElementById('styleStatsList');if(styleStatsList&&stats.style_counts){const styleEntries=Object.entries(stats.style_counts);if(styleEntries.length>0){const allStyles=styleEntries;styleStatsList.innerHTML=allStyles.map(([style,count])=>`
...
try{const searchLower=this.currentSearch.toLowerCase();const styleKeywords=['style:','genre:','type:'];const isStyleSearch=styleKeywords.some(keyword=>searchLower.startsWith(keyword));const searchParam=isStyleSearch?'':this.currentSearch;const params=new URLSearchParams({action:'albums',filter:this.currentFilter,search:searchParam});const response=await this.fetchWithCache(`api/music_api.php?${params}`);const data=await response.json();if(data.success){let albums=data.data;if(this.currentStyleFilter){albums=albums.filter(album=>{if(!album.style)return!1;const styles=album.style.split(',').map(s=>s.trim());return styles.includes(this.currentStyleFilter)})}
...
this.renderAlbums(albums)}else{this.showMessage('Error loading albums: '+data.message,'error')}}catch(error){this.showMessage('Error loading albums','error')}finally{this.hideLoading();if(tableContainer){tableContainer.classList.remove('loading')}}}
...
async fetchMasterYearForSelection(releaseId,yearInput){try{const response=await this.fetchWithCache(`api/music_api.php?action=master_year&release_id=${releaseId}`);const data=await response.json();if(data.success&&data.data&&data.data.master_year){if(yearInput){yearInput.value=data.data.master_year}}}catch(error){console.log('Error fetching master year for selection:',error)}}
...
async deleteAlbum(id){console.log('Delete album called with ID:',id);console.log('Authentication status:',this.isAuthenticated);if(!this.isAuthenticated){console.log('Not authenticated, showing login modal');this.showLoginModal();return}
if(!confirm('Are you sure you want to delete this album?')){console.log('User cancelled deletion');return}
console.log('Making delete request...');try{const controller=new AbortController();const timeoutId=setTimeout(()=>controller.abort(),10000);const response=await fetch('api/music_api.php?action=delete',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:id}),signal:controller.signal});clearTimeout(timeoutId);console.log('Response received:',response.status,response.statusText);const data=await response.json();console.log('Response data:',data);if(data.success){console.log('Delete successful');this.showMessage('Album deleted successfully','success');this.loadAlbums();this.loadStats()}else{if(data.auth_required){console.log('Authentication required, showing login modal');this.showLoginModal()}else{console.log('Delete failed:',data.message);this.showMessage('Error deleting album: '+data.message,'error')}}}catch(error){console.error('Delete request failed:',error);if(error.name==='AbortError'){this.showMessage('Delete request timed out. Please try again.','error')}else{this.showMessage('Error deleting album','error')}}}
...
params.append('album_id',albumId);const response=await this.fetchWithCache(`api/tracklist_api.php?${params}`);const data=await response.json();if(data.success&&data.data&&data.data.master_year){const yearElement=info.querySelector('.album-year');if(yearElement){yearElement.textContent=data.data.master_year}}}catch(error){console.log('Could not fetch master release year:',error)}}}
...
this.checkPasswordStatus();document.getElementById('resetPasswordMessage').style.display='none';document.getElementById('resetPasswordForm').reset();document.getElementById('resetPasswordModal').style.display='block';document.getElementById('reset_current_password').focus()}
...
async checkPasswordStatus(){try{const response=await fetch('api/music_api.php?action=auth_check');const data=await response.json();if(data.success){const statusText=document.getElementById('passwordStatusText');const statusDiv=document.getElementById('passwordStatus');if(data.data.authenticated){statusText.textContent='Set';statusDiv.className='password-status set'}else{statusText.textContent='Not set';statusDiv.className='password-status not-set'}}}catch(error){console.error('Error checking password status:',error)}}
async handleResetPassword(event){event.preventDefault();const currentPassword=document.getElementById('reset_current_password').value;const newPassword=document.getElementById('reset_new_password').value;const confirmPassword=document.getElementById('reset_confirm_password').value;const messageDiv=document.getElementById('resetPasswordMessage');try{const response=await fetch('api/music_api.php?action=reset_password',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({current_password:currentPassword,new_password:newPassword,confirm_password:confirmPassword})});const data=await response.json();if(data.success){messageDiv.textContent=data.message;messageDiv.className='modal-message success';messageDiv.style.display='block';document.getElementById('resetPasswordForm').reset();this.checkPasswordStatus();setTimeout(()=>{this.hideResetPasswordModal()},3000)}else{messageDiv.textContent=data.message;messageDiv.className='modal-message error';messageDiv.style.display='block'}}catch(error){messageDiv.textContent='Network error. Please try again.';messageDiv.className='modal-message error';messageDiv.style.display='block'}}
showSetupModal(){if(!this.isAuthenticated){this.showLoginModal();return}
this.loadSetupStatus();document.getElementById('setupMessage').style.display='none';document.getElementById('setupForm').reset();document.getElementById('setupModal').style.display='block';document.getElementById('setup_discogs_api_key').focus()}
...
const passwordStatus=document.getElementById('passwordStatus');const passwordActionText=document.getElementById('passwordActionText');if(statusData.password_set){passwordStatus.textContent='✅ Set';passwordStatus.className='status-value set';passwordActionText.textContent='Change Password'}else{passwordStatus.textContent='❌ Not set';passwordStatus.className='status-value not-set';passwordActionText.textContent='Set Password'}
const overallStatus=document.getElementById('overallStatus');if(statusData.setup_complete){overallStatus.textContent='✅ Complete';overallStatus.className='status-value set'}else{overallStatus.textContent='⚠️ Incomplete';overallStatus.className='status-value not-set'}}}catch(error){console.error('Error loading setup status:',error)}}
...
hideLoginModal(){console.log('hideLoginModal called');const modal=document.getElementById('loginModal');if(modal){modal.style.display='none';console.log('Login modal hidden')}else{console.error('Login modal element not found')}
...
async handleLogin(event){event.preventDefault();console.log('Login attempt started');const password=document.getElementById('password').value;const messageDiv=document.getElementById('loginMessage');if(!password){messageDiv.textContent='Please enter a password.';messageDiv.className='modal-message error';messageDiv.style.display='block';return}
try{console.log('Making login request...');const controller=new AbortController();const timeoutId=setTimeout(()=>controller.abort(),10000);const response=await fetch('api/music_api.php?action=login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({password:password}),signal:controller.signal});clearTimeout(timeoutId);console.log('Login response status:',response.status,response.statusText);const data=await response.json();console.log('Login response data:',data);if(data.success){console.log('Login successful');this.isAuthenticated=!0;console.log('Updated isAuthenticated to:',this.isAuthenticated);this.updateAuthUI();console.log('Updated auth UI');this.hideLoginModal();console.log('Hidden login modal');this.showMessage('Login successful','success')}else{console.log('Login failed:',data.message);messageDiv.textContent=data.message;messageDiv.className='modal-message error';messageDiv.style.display='block'}}catch(error){console.error('Login request failed:',error);if(error.name==='AbortError'){messageDiv.textContent='Login request timed out. Please try again.'}else{messageDiv.textContent='Login failed. Please try again.'}
messageDiv.className='modal-message error';messageDiv.style.display='block'}}
