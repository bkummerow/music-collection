...
async checkAuthStatus(){try{const response=await this.fetchWithCache('api/music_api.php?action=auth_status');const data=await response.json();if(data.success){this.isAuthenticated=data.data.authenticated;this.updateAuthUI()}}catch(error){console.error('Error checking auth status:',error)}}
updateAuthUI(){const addBtn=document.getElementById('addAlbumBtn');const logoutBtn=document.getElementById('logoutBtn');if(addBtn){if(this.isAuthenticated){addBtn.textContent='+ Add Album';addBtn.style.opacity='1';addBtn.style.cursor='pointer'}else{addBtn.textContent='+ Add Album (Login Required)';addBtn.style.opacity='0.7';addBtn.style.cursor='pointer'}}
...
this.showAutocompleteLoading('artistAutocomplete');try{const url=`api/music_api.php?action=artists&search=${encodeURIComponent(search)}`;const response=await this.fetchWithCache(url);const data=await response.json();if(data.success){this.showAutocomplete('artistAutocomplete',data.data,'artist_name')}else{console.error('API returned error:',data.message);this.hideAutocomplete('artistAutocomplete')}}catch(error){console.error('Error fetching artists:',error);this.hideAutocomplete('artistAutocomplete')}}
async handleAlbumAutocomplete(artist,search){if(search.length<2){this.hideAutocomplete('albumAutocomplete');return}
this.showAutocompleteLoading('albumAutocomplete');try{const url=`api/music_api.php?action=albums_by_artist&artist=${encodeURIComponent(artist)}&search=${encodeURIComponent(search)}`;const response=await this.fetchWithCache(url);const data=await response.json();if(data.success){this.showAutocomplete('albumAutocomplete',data.data,'album_name')}else{console.error('API returned error:',data.message);this.hideAutocomplete('albumAutocomplete')}}catch(error){console.error('Error fetching albums:',error);this.hideAutocomplete('albumAutocomplete')}}
...
if(!list){return}
list.innerHTML='';items.forEach(item=>{if(item&&item[field]&&item[field]!=='undefined'){const div=document.createElement('div');div.className='autocomplete-item';if(field==='album_name'&&item.year){div.textContent=`${item[field]} (${item.year})`}else{div.textContent=item[field]}
if(item.cover_url){const coverImg=document.createElement('img');coverImg.src=item.cover_url;coverImg.className='autocomplete-cover';coverImg.onerror=()=>{coverImg.remove()};coverImg.onload=()=>{};div.appendChild(coverImg)}
...
if(list){list.innerHTML='<div class="autocomplete-loading">Searching...</div>';list.style.display='block'}else{console.error('Could not find autocomplete list for:',containerId)}}
...
if(list){list.style.display='none';list.style.visibility='hidden';list.style.opacity='0';if(list.parentNode===document.body&&list.dataset.originalContainer){const originalContainer=document.getElementById(list.dataset.originalContainer);if(originalContainer){originalContainer.appendChild(list);list.style.position='';list.style.top='';list.style.left='';list.style.width='';list.style.zIndex='';delete list.dataset.originalContainer}}}else{console.log('No list found to hide for:',containerId)}}
...
async loadStats(){try{const response=await this.fetchWithCache('api/music_api.php?action=stats');const data=await response.json();if(data.success){this.updateStats(data.data)}}catch(error){console.error('Error loading stats:',error)}}
...
try{const params=new URLSearchParams({action:'albums',filter:this.currentFilter,search:this.currentSearch});const response=await this.fetchWithCache(`api/music_api.php?${params}`);const data=await response.json();if(data.success){this.renderAlbums(data.data)}else{this.showMessage('Error loading albums: '+data.message,'error')}}catch(error){console.error('Error loading albums:',error);this.showMessage('Error loading albums','error')}finally{this.hideLoading();if(tableContainer){tableContainer.classList.remove('loading')}}}
...
async editAlbum(id){try{const response=await this.fetchWithCache(`api/music_api.php?action=album&id=${id}`);const data=await response.json();if(data.success){this.editingAlbum=data.data;this.showModal(data.data)}else{this.showMessage('Error loading album: '+data.message,'error')}}catch(error){console.error('Error loading album:',error);this.showMessage('Error loading album','error')}}
async deleteAlbum(id){if(!this.isAuthenticated){this.showLoginModal();return}
if(!confirm('Are you sure you want to delete this album?')){return}
try{const response=await fetch('api/music_api.php?action=delete',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:id})});const data=await response.json();if(data.success){this.showMessage('Album deleted successfully','success');this.loadAlbums();this.loadStats()}else{if(data.auth_required){this.showLoginModal()}else{this.showMessage('Error deleting album: '+data.message,'error')}}}catch(error){console.error('Error deleting album:',error);this.showMessage('Error deleting album','error')}}
showModal(album=null){const modal=document.getElementById('albumModal');const form=document.getElementById('albumForm');const title=document.querySelector('#albumModal h2');form.reset();this.hideModalMessage();if(album){title.textContent='Edit Album';document.getElementById('artistName').value=album.artist_name;document.getElementById('albumName').value=album.album_name;document.getElementById('releaseYear').value=album.release_year||'';if(album.is_owned==1){document.getElementById('isOwned').checked=!0}else if(album.want_to_own==1){document.getElementById('wantToOwn').checked=!0}
const albumInput=document.getElementById('albumName');albumInput.disabled=!1;albumInput.placeholder='Enter album name...'}else{title.textContent='Add New Album';this.editingAlbum=null;this.updateAlbumInputState()}
...
try{const response=await fetch(`api/music_api.php?action=${action}`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(albumData)});const data=await response.json();if(data.success){this.hideModal();this.showMessage(data.message,'success');this.loadAlbums();this.loadStats();this.editingAlbum=null}else{if(data.auth_required){this.showLoginModal()}else{this.showModalMessage(data.message,'error')}}}catch(error){console.error('Error saving album:',error);this.showModalMessage('Error saving album','error')}}
...
async showTracklist(artistName,albumName,releaseYear,albumId=null){const modal=document.getElementById('tracklistModal');const title=document.getElementById('tracklistModalTitle');const info=document.getElementById('tracklistModalInfo');const tracks=document.getElementById('tracklistModalTracks');const discogsLink=document.getElementById('tracklistModalDiscogsLink');const coverImage=document.getElementById('tracklistModalCover');const noCover=document.getElementById('tracklistModalNoCover');title.textContent=`${albumName}`;info.innerHTML=`
        ...
        `;coverImage.style.display='none';noCover.style.display='flex';tracks.innerHTML='<div class="tracklist-loading">Loading tracklist...</div>';modal.style.display='block';try{const params=new URLSearchParams({artist:artistName,album:albumName});if(releaseYear){params.append('year',releaseYear)}
if(albumId){params.append('album_id',albumId)}
const response=await this.fetchWithCache(`api/tracklist_api.php?${params}`);const data=await response.json();if(data.success&&data.data){const albumData=data.data;let formattedReleased='';if(albumData.released){try{const date=new Date(albumData.released);if(!isNaN(date.getTime())){formattedReleased=date.toLocaleDateString('en-US',{month:'short',day:'numeric',year:'numeric'})}else{formattedReleased=albumData.released}}catch(e){formattedReleased=albumData.released}}
info.innerHTML=`
                ...
                `;if(albumData.cover_url){const coverUrl=albumData.cover_url_medium||albumData.cover_url;coverImage.src=coverUrl;coverImage.style.display='block';noCover.style.display='none';coverImage.onerror=function(){coverImage.style.display='none';noCover.style.display='flex'};coverImage.onload=function(){coverImage.classList.add('loaded')}}else{coverImage.style.display='none';noCover.style.display='flex'}
...
async handleLogout(){try{const response=await fetch('api/music_api.php?action=logout',{method:'POST',headers:{'Content-Type':'application/json'}});const data=await response.json();if(data.success){this.isAuthenticated=!1;this.updateAuthUI();this.showMessage('Logged out successfully','success')}}catch(error){console.error('Logout error:',error)}}
...
loadOptimizedImage(img,src,fallbackSrc=null){const webpSupported=this.isWebPSupported();if(webpSupported&&src.includes('discogs.com')&&!src.includes('fm=webp')){const webpSrc=src.includes('?')?src+'&fm=webp':src+'?fm=webp';img.src=webpSrc;img.onerror=()=>{img.src=src}}else{img.src=src}
if(fallbackSrc){img.onerror=()=>{img.src=fallbackSrc}}}
