...
return!0})}
const sortedAlbums=this.sortAlbums(albums);this.renderAlbums(sortedAlbums)}else{this.showMessage('Error loading albums: '+data.message,'error')}}catch(error){this.showMessage('Error loading albums','error')}finally{this.hideLoading();if(tableContainer){tableContainer.classList.remove('loading')}}}
...
renderAlbumsWithSort(){const tbody=document.querySelector('#albumsTable tbody');const rows=Array.from(tbody.querySelectorAll('tr'));const albums=rows.map(row=>{if(row.classList.contains('empty-state'))return null;const artistName=row.querySelector('.artist-name')?.textContent||'';const albumName=row.querySelector('.album-name a')?.textContent||'';const yearElement=row.querySelector('.year-link');const releaseYear=yearElement?yearElement.dataset.year:'';const isOwned=row.querySelector('td:nth-child(4) .checkmark')?.textContent==='✓';const wantToOwn=row.querySelector('td:nth-child(5) .checkmark')?.textContent==='✓';return{id:row.dataset.id,artist_name:artistName,album_name:albumName,release_year:releaseYear,is_owned:isOwned?1:0,want_to_own:wantToOwn?1:0,cover_url:row.querySelector('.album-cover')?.dataset.src||'',cover_url_medium:row.querySelector('.album-cover')?.dataset.medium||'',cover_url_large:row.querySelector('.album-cover')?.dataset.large||''}}).filter(album=>album!==null);const sortedAlbums=this.sortAlbums(albums);this.renderAlbums(sortedAlbums)}
sortAlbums(albums){return albums.sort((a,b)=>{if(this.currentSort.field==='year'){const yearA=parseInt(a.release_year)||0;const yearB=parseInt(b.release_year)||0;if(this.currentSort.direction==='desc'){if(yearA!==yearB){return yearB-yearA}
return a.artist_name.localeCompare(b.artist_name)}else{if(yearA!==yearB){return yearA-yearB}
return a.artist_name.localeCompare(b.artist_name)}}else if(this.currentSort.field==='album'){const artistComparison=a.artist_name.localeCompare(b.artist_name);if(this.currentSort.direction==='desc'){if(artistComparison!==0){return-artistComparison}
return a.album_name.localeCompare(b.album_name)}else{if(artistComparison!==0){return artistComparison}
return a.album_name.localeCompare(b.album_name)}}else{return a.artist_name.localeCompare(b.artist_name)}})}}
