...
async checkAuthStatus(){try{const response=await this.fetchWithCache('api/music_api.php?action=auth_status');const data=await response.json();if(data.success){this.isAuthenticated=data.data.authenticated}}catch(error){this.isAuthenticated=!1}
this.updateAuthUI()}
...
const response=await this.fetchWithCache(`api/tracklist_api.php?${params}`);const data=await response.json();if(data.success&&data.data){const albumData=data.data;let formattedReleased='';if(albumData.released){try{const date=new Date(albumData.released);if(!isNaN(date.getTime())){const month=date.getMonth();const day=date.getDate();if(!(month===11&&day===31)){formattedReleased=date.toLocaleDateString('en-US',{month:'short',day:'numeric',year:'numeric'})}}else{formattedReleased=albumData.released}}catch(e){formattedReleased=albumData.released}}
info.innerHTML=`
              ...
              `;if(albumData.cover_url){const coverUrl=albumData.cover_url_medium||albumData.cover_url;noCover.textContent='Loading Cover...';noCover.style.display='flex';coverImage.style.display='none';coverImage.src=coverUrl;coverImage.onload=function(){coverImage.style.display='block';noCover.style.display='none';coverImage.classList.add('loaded')};coverImage.onerror=function(){coverImage.style.display='none';noCover.textContent='No Cover';noCover.style.display='flex'}}else{coverImage.style.display='none';noCover.textContent='No Cover';noCover.style.display='flex'}
