...
if(item){if(containerId==='artistAutocomplete'){this.selectedArtist=item}else if(containerId==='albumAutocomplete'){this.selectedAlbum=item;this.selectedDiscogsReleaseId=item.id||null;this.selectedCoverUrl=item.cover_url||null;const yearInput=document.getElementById('releaseYear');if(yearInput){yearInput.value=item.year||''}
if(item.id){this.fetchMasterYearForSelection(item.id,yearInput)}}}}
...
async fetchMasterYearForSelection(releaseId,yearInput){try{const response=await this.fetchWithCache(`api/tracklist_api.php?release_id=${releaseId}`);const data=await response.json();if(data.success&&data.data&&data.data.master_year){if(yearInput){yearInput.value=data.data.master_year}}}catch(error){console.log('Error fetching master year for selection:',error)}}
...
showStatsModal(){this.loadStats();document.getElementById('statsModal').style.display='block'}
